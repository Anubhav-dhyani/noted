...
An Android-first two-person private messaging app. One person creates a permanent six-digit room,
the other joins it, and messages appear on both phones after the sender taps **Send**. Draft text
stays local. The room remains valid across app, phone, and backend restarts until either participant
logs out. When the receiving app is backgrounded, the backend sends a heads-up push notification
